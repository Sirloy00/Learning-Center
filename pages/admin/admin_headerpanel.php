<?php if($_SESSION['role'] == "administrator")
							{								
								...
								$_SESSION['admin_username'] = $row['uname'];
								$_SESSION['admin_pass'] = $row['password'];
								$_SESSION['admin_usern'] = $row['username'];
								$_SESSION['admin_id'] = $row['id'];
								$_SESSION['admin_images'] = $row['images'];
				             
								}							
							}
	$admin_username = $_SESSION['admin_username'];	 	
    $tempusername = $admin_username;	
 ?>
  <?php if($_SESSION['role'] == "administrator")
							{								
								$user2 = mysqli_query($con,"SELECT *,CONCAT(lname,', ', fname,' ', mi)  as uname from tbladmin where id = '".$_SESSION['userid']."' ");
								while($row = mysqli_fetch_array($user2)){
								$_SESSION['admin_username2'] = $row['uname'];
								$_SESSION['admin_id'] = $row['id'];
								}							
							} 
      ...
      <ul class="navbar-nav ml-auto ml-md-0">
        ...
        <li class="nav-item dropdown no-arrow mx-1" style="margin-top: 5px;">
          <a class="nav-link dropdown-toggle"  title="Messages" href="?update='true'" id="messagesDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
			...
			<?php
                      ...
                      $q = mysqli_query($con, "select * from message where receiver_name = 'administrator' and message_status='unread' ");
                        $num_rows = mysqli_num_rows($q);
						$_SESSION['admin_message_count'] = $num_rows;
						if($_SESSION['admin_message_count'] <= 9 and $_SESSION['admin_message_count'] >= 1)
						{
							echo'<span class="badge badge-danger" style="font-size:9px">
						     '.$_SESSION['admin_message_count'].'
							</span>';
						}
						else if($_SESSION['admin_message_count']<=0)
						{
							echo'';
						}
						else if($_SESSION['admin_message_count']>=10)
						{
							echo'<span class="badge badge-danger" style="font-size:9px">
						     9+
							</span>';
          ...
          <div class="dropdown-menu dropdown-menu-right" aria-labelledby="messagesDropdown" style = "border:solid 1px gray;">
			...
			<a href="compose_message.php"  title="Compose a Message!" style="float:right;"><i class="fas fa-fw fa-edit" style="font-size:23px;color:white"></i></a></h4>
			</div>
		    <div style="height:400px;width:400px;border:solid 0.5px gray;overflow:scroll;overflow-x:hidden;overflow-y:scroll;"><br/>
		    <?php
			                if($_SESSION['admin_message_count'] == 0)
								{
									echo '<br/><br/></br></br><center><h6>No message found!</h6><center>';
								}
						    $squery = mysqli_query($con, "select * from message where receiver_name = 'administrator' and message_status = 'unread' order by message_id DESC");
							while($row = mysqli_fetch_array($squery))
							{
								...
								$_SESSION['sender_name'] = $row['sender_name'];
									...
									$_SESSION['message_id']=$row['message_id'];
								 	 ?>
								   <?php 	echo'
									  <a a class="dropdown-item" title="Click here to view conversation!" data-toggle="modal" data-target="#adminreply'.$_SESSION['message_id'].'">
           ...
           ?>  
            ...
            <a class="dropdown-item" href="teacher_message_inbox.php">
			 ...
			 <a class="dropdown-item" href = "" data-toggle="modal" data-target="#admin_markall_messageModal">
		...
		<?php include "admin_mark_all_message.php"; ?>
		<!--profile-->
        <li class="nav-item dropdown no-arrow">
          <a class="nav-link dropdown-toggle" title="User Account" href="#" id="userDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
           <?php echo'<img src="'.$_SESSION['admin_images'].'" style = " border-radius: 100%;height: 30px; width: 30px;border: 2px solid skyblue;"/>';?>
          </a>
          <?php 
		  echo'<div class="dropdown-menu dropdown-menu-right" aria-labelledby="userDropdown">';
				if($_SESSION['role'] == "administrator"){
			        $user = mysqli_query($con,"SELECT *,CONCAT(fname,' ', mi,' ', lname)  as uname from tbladmin where id = '".$_SESSION['admin_id']."' ");
                    while($row = mysqli_fetch_array($user)){
                        $_SESSION['user'] = "Administrator" ;
						echo' <a class="dropdown-item" href="#"><center><img src="'.$_SESSION['admin_images'].'" style = " border-radius: 100%;height: 80px; width: 80px; border: 2px solid skyblue;"</center><b><center><span style="font-size:15px">'.$row['uname'].'</span></b><br/></a>';
                        }
                    }
           echo' 
		   ...
		   <a data-toggle="modal" data-target="#userprofileModal" class="dropdown-item" href="#" style="color:blue;"><i class="fas fa-fw fa-edit" style="font-size:12px"></i>Edit User Profile</a>
            <div class="dropdown-divider"></div>
            <a class="dropdown-item" href="#" data-toggle="modal" data-target="#adminlogoutModal"><center><span style="color: blue">Log out</span> </center></a>
          </div>';
		  ?>
        </li>
		<?php include"admin_userprofileModal.php"?>
      </ul>
