        ...
        <div class="container-fluid">
          ...
          <div class="card mb-3">
            <div class="card-header" style = "background-color:rgb(51, 58, 65);color:white;">
              <h4><i class="fas fa-gear"></i>
			 Manage Feedback</h4></div>
			  <div class = "container" >
				<br/>
				<div class= "table-responsive">
					<table id="tblfeedback" style="border:solid 1px green;"class="table table-striped table-bordered">
				    <thead style="background-color: green;color:white;">
						...
						<tbody>
						  <?php
								$num=1;
                                $squery = mysqli_query($con, "select * from tblfeedback order by id DESC");
                                while($row = mysqli_fetch_array($squery))
                                {
                                    ...
                                    <tr>
										<td>
											<center>
											<button name="Edit" class="btn btn-danger btn-sm" data-target="#feedbackdeleteModal'.$row['id'].'" data-toggle="modal"><i class="far fa-trash-alt"></i></button></center>
										</td>
										<td>'.$num.'</td>
										<td><img src="'.$row['sender_image'].'"style="height:70px; width:90px;"/></td>
										<td>'.$row['sender_name'].'</td>
										<td>'.$row['message'].'</td>
										<td>'.$row['date_sent'].'</td>
							        </tr>
                                     ';		
                                    include"admin_feedback_DeleteModal.php";									 
                                 $num++;             
                                }
                           ?>
						</tbody>
